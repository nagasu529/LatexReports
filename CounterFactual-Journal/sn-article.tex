...
\subsection{Predictive Analytics and Explainable AI in Agriculture}
Machine learning has been widely adopted in agricultural analytics, driving advancements in crop yield prediction, resource optimization, and farmer segmentation \cite{placeholder1_ml_agri}. However, the inherent ``black-box'' nature of advanced models like Random Forests and Deep Learning limits their interpretability, prompting a significant shift towards eXplainable AI (XAI) \cite{placeholder2_xai_need}. Techniques such as SHapley Additive exPlanations (SHAP) and Local Interpretable Model-agnostic Explanations (LIME) have been extensively employed to identify the key drivers of agricultural productivity and technology adoption \cite{placeholder3_shap_agri}. While these methods successfully highlight global and local feature importance—revealing \textit{what} factors influence a farmer's classification—they inherently lack actionability. They describe the current state but cannot provide a concrete, step-by-step pathway for a farmer to transition from a low-skill to a high-skill segment.

\subsection{Counterfactual Explanations as Actionable AI}
Counterfactual Explanations (CFs) address the limitations of traditional feature-attribution XAI by answering personalized ``what-if'' scenarios \cite{placeholder4_cf_intro}. Instead of merely assigning importance weights, CFs generate a hypothetical profile demonstrating the minimal feature perturbations required to alter a model's prediction toward a desired outcome \cite{placeholder5_cf_theory}. While CFs have seen rapid adoption and maturity in domains requiring strict algorithmic recourse, such as finance (e.g., credit scoring) and healthcare \cite{placeholder6_cf_finance}, their application in smart agriculture and extension services remains largely unexplored. By applying CFs, agricultural recommendations can shift from generalized policies to highly personalized, actionable roadmaps.

\subsection{Algorithmic Constraints and Optimization}
Generating realistic counterfactuals is essentially an optimization problem. Standard CF generation methods, such as gradient descent or random sampling, often ignore domain-specific realities \cite{placeholder7_cf_optimization}. In real-world agricultural interventions, strict constraints must be enforced; for instance, a farmer's demographic attributes (e.g., age) are immutable, and skill levels must follow monotonic constraints (i.e., skills can increase through training but cannot practically decrease to achieve a target class). Recent advancements in Computer Science highlight the efficacy of evolutionary algorithms, such as Genetic Algorithms (GA), in navigating non-differentiable constraint spaces to find optimal, sparse counterfactuals \cite{placeholder8_ga_cf}. This paper bridges the interdisciplinary gap by applying a GA-constrained CF framework to generate actionable, monotonically feasible agricultural training paths.
...
To operationalize the counterfactuals, the generated feature perturbations were mapped to corresponding agricultural extension courses. As illustrated in Figure X, analysis of the transition paths reveals distinct structural bottlenecks depending on the farmer's current skill tier. 

For the Low-to-Moderate transition (C0 $\rightarrow$ C1), ``Marketing \& Value Addition'' and ``Input \& Resource Optimization'' emerged as the most frequently recommended interventions. In contrast, the Moderate-to-High transition (C1 $\rightarrow$ C2) displays a drastic shift in required competencies. ``Agri-Business Analysis'' overwhelmingly dominates the recommendations for this advanced transition, supplemented by continued needs in ``Marketing \& Value Addition'' and a significant increase in ``Network Building''.

\section{Discussion and Conclusion}\label{sec5}

\subsection{Discussion}
The integration of a Genetic Algorithm-based Counterfactual framework provides a substantial advancement over traditional agricultural segmentation methods. The quantitative evaluation demonstrates that the generated transition paths are highly practical, requiring farmers to focus on only a sparse number of specific training domains.

More importantly, the algorithmic insights generated by our framework align perfectly with the real-world trajectory of agricultural economic maturation. For low-skill farmers, the immediate bottlenecks are foundational: optimizing resource inputs and securing basic market access. However, the interventions required to elevate a farmer from moderate to high-skill are fundamentally different. At this advanced stage, physical production capabilities are no longer the primary constraint. Instead, farmers require advanced ``Agri-Business Analysis'' and networking capabilities to scale their operations and negotiate better market positions. 

This stark contrast between the C0 $\rightarrow$ C1 and C1 $\rightarrow$ C2 transition paths demonstrates that our CF framework does not merely output random mathematical perturbations. Instead, it captures the true developmental trajectory of agricultural human capital, providing extension officers with a robust, data-driven tool to allocate specialized training resources exactly where they are needed most.
