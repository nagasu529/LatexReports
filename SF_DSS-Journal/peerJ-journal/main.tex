...
We used a Random Forest regression model \citep{breiman2001} to predict skill change ($Ch\_Skill$). Since each farmer appears twice in the dataset (before and after training), we used 5-fold GroupKFold cross-validation, grouping by farmer ID. This ensures the model is not tested on data from a farmer it has already seen during training. We also trained an XGBoost model \citep{chen2016} as a robustness check. 

We selected Random Forest settings through a grid search over \texttt{n\_estimators}, \texttt{max\_depth}, \texttt{min\_samples\_leaf}, and \texttt{max\_features}. The final settings were \texttt{n\_estimators} $= 500$, \texttt{min\_samples\_leaf} $= 10$, and \texttt{max\_features} $=$ \texttt{sqrt}. 
